...
const STATUS_META = {
    ...
    IN_PROGRESS: { label: "Watching", color: "bg-yellow-500" },
...
export default function Watchlist() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                const message = error.response?.data?.message || "Failed to load watchlist.";
                toast.error(message);
            } finally {
                setLoading(false);
            }
    ...
    const handleRemove = useCallback(async (e, item) => {
        e.stopPropagation();
        const prev = [...items];
        const name = typeof item === "object" ? item.movieTitle : "Item";
        const movieId = typeof item === "object" ? item.movieId : item;
